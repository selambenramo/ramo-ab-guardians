...
mongoose.connect('mongodb+srv://ingiltereliguard:DzAlcDLv5Ak4Mext@cluster0.ilg44.azure.mongodb.net/ingiltereliguard?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});
...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "Witcher 💜 Stgmamy",}, status: "online"});
...
client.on("message", async message => {
  ...
  if (command === "koruma")  {
    if (!args[0] || args[0] !== "rol") return message.channel.send(embed.setDescription(`Rol korumasını aktif etmek veya devre dışı bırakmak için \`\`${ayarlar.botPrefix}koruma rol\`\` yazmanız yeterlidir! Rol koruması şu anda \n\`\`${ayarlar.roleGuard ? "aktif" : "devre dışı"}!\`\``));
  ...
  if (command === "kur" || command === "kurulum" || command === "setup") {
    ...
    Database.findOne({guildID: ayarlar.guildID, roleID: args[0]}, async (err, roleData) => {
      ...
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('STGXMAMY').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`STGXMAMY`).setTimestamp()).catch(); } else { message.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Rol Yedeği Kullanıldı!').setDescription(`${message.author} (${message.author.id}) tarafından **${roleData.name} (${roleData.roleID})** rolünün yedeği kurulmaya başlandı! Rol tekrar oluşturularak, üyelerine dağıtılmaya ve izinleri kanallara eklenmeye başlanıyor.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "STGXMAMY"}& Witcher`).setTimestamp()).catch(err => {}); };
    });
  };
});
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "STGXMAMY" }).catch();
};
...
client.on("guildMemberUpdate", async (oldMember, newMember) => {
  if (newMember.roles.cache.size > oldMember.roles.cache.size) {
    ...
    if (yetkiPermleri.some(p => !oldMember.hasPermission(p) && newMember.hasPermission(p))) {
      ...
      let logKanali = client.channels.cache.get(ayarlar.logChannelID);
      if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('STGXMAMY').setDescription(`${newMember},(\`\`${newMember.id}\`\`) üyesine ${entry.executor},(\`\`${entry.executor.id}\`\`) tarafından sağ tık yetki verildi! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "Percy"} was here!`).setTimestamp()).catch(); } else { newMember.guild.owner.send(new MessageEmbed().setColor("2F3136").setTitle('Sağ Tık Yetki Verildi!').setDescription(`${newMember} (${newMember.id}) üyesine ${entry.executor} (${entry.executor.id}) tarafından sağ tık yetki verildi! Veren kişi yasaklandı ve verilen kişiden rol geri alındı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "STGXMAMY"}& Witcher`).setTimestamp()).catch(err => {}); };
    };
  };
});
...
client.on("roleCreate", async role => {
  ...
  role.delete({ reason: "STGXMAMY rol koruma" });
...
function setRoleBackup() {
  ...
  if (guild) {
    ...
    console.log(`STGXMAMY rol yedeği başarılı bir şekilde alındı ${moment(Date.now()).format("DD")} ${aylar[moment(Date.now()).format("MM")]} ${moment(Date.now()).format("YYYY HH:mm:ss")} `);
  };
};
